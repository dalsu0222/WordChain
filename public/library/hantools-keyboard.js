...
(function () {
  ...
  var _qwerty_key = function (c) {
    ...
    _qwerty_key_inv = function (key) {
      ...
      else if (key[1] && key[0] in QWERTY_SHIFT) return QWERTY_SHIFT[key[0]];
      else return key[0];
    };
  ...
  var SEBEOLSIK_390_MAP = [
    "k kk h u uu y i ; ;; n nn j l ll o 0 ' p m"
      .split(" ")
      .map(_key_map_convert),
    "f r 6 R t c e 7 v /f /r /d 4 b 9t 9c 9d 5 g 8 d"
      .split(" ")
      .map(_key_map_convert),
    " x F xq s s! S A w D C w3 wq wW wQ V z 3 X q 2 a ! Z E W Q 1"
      .split(" ")
      .map(_key_map_convert),
  ];
  ...
  var HanTools_Keyboard = function INIT_HANTOOLS_KEYBOARD(HanTools) {
    ...
    Keyboard.prototype.getBufferString = function () {
      return this.buffer.join("");
    };
    Keyboard.prototype.getString = function () {
      return this.output + this.getBufferString();
    };
    Keyboard.prototype.flushBuffer = function () {
    ...
    Keyboard.DUBEOLSIK.getKeySequence = function DUBEOLSIK_getKeySequence(str) {
      ...
      str
        .split("")
        .map(HanTools.disintegrate)
        .forEach(function (part) {
          var ind;
          if (typeof part == "string")
            _push_multi(seq, _kor_key(DUBEOLSIK_MAP, part));
          else
            part.forEach(function (c) {
              _push_multi(seq, _kor_key(DUBEOLSIK_MAP, c));
            });
        });
      return seq;
    };
    Keyboard.DUBEOLSIK.type = Keyboard.type.bind(Keyboard.DUBEOLSIK);
    Keyboard.DUBEOLSIK.prototype.getBufferString =
      function DUBEOLSIK$getBufferString() {
    ...
    Keyboard.DUBEOLSIK.prototype.type = function DUBEOLSIK$type(seq) {
      seq.forEach(function (key) {
        ...
        if ("a" <= ch && ch <= "z") {
          ...
          var p,
            _c = _composible_hangul(this.buffer[this.buffer.length - 1], ch),
          ...
          if (v) {
            ...
            else if (this.buffer.length == 2 || (this.buffer.length == 3 && _c))
              this.buffer.push(ch);
            else {
              this.flushBuffer();
              this.buffer.push(ch);
    ...
    Keyboard.SEBEOLSIK_390.getKeySequence =
      function SEBEOLSIK_390_getKeySequence(str) {
        var seq = [];
        str
          .split("")
          .map(HanTools.disintegrate)
          .forEach(function (part) {
            ...
            if (typeof part == "string") {
              switch (part) {
                case "ㅘ":
                  seq.push(["v", false], ["f", false]);
                  break;
                case "ㅙ":
                  seq.push(["v", false], ["r", false]);
                  break;
                case "ㅚ":
                  seq.push(["v", false], ["d", false]);
                  break;
                case "ㅝ":
                  seq.push(["b", false], ["f", false]);
                  break;
                case "ㅞ":
                  seq.push(["b", false], ["c", false]);
                  break;
                case "ㅟ":
                  seq.push(["b", false], ["d", false]);
                  break;
                default:
                  _push_multi(seq, _kor_key(SEBEOLSIK_390_MAP, part));
              }
            } else {
              _push_multi(
                seq,
                SEBEOLSIK_390_MAP[0][HanTools.CHOSEONG.indexOf(part[0])]
              );
              _push_multi(
                seq,
                SEBEOLSIK_390_MAP[1][HanTools.JUNGSEONG.indexOf(part[1])]
              );
              if (part.length == 3)
                _push_multi(
                  seq,
                  SEBEOLSIK_390_MAP[2][HanTools.JONGSEONG.indexOf(part[2])]
                );
            }
          });
        return seq;
    ...
    Keyboard.SEBEOLSIK_390.prototype.getBufferString =
      function SEBEOLSIK_390$getBufferString() {
        return HanTools.compose(this.buffer);
      };
        ...
        if (part == 0) {
          ...
          if (this.buffer[0]) {
            _c =
              !this.buffer[1] &&
              this.buffer[0] == ch &&
              _composible_hangul(this.buffer[0], ch);
          ...
          if (this.buffer[2]) {
            _c =
              this.buffer[2] != ch || ch == "ㄱ" || ch == "ㅅ"
                ? _composible_hangul(this.buffer[2], ch)
                : null;
